...
#let dospelak(data) = [
  ...
  #form([
    ...
    #field("Datum narození:", to: 35%, content: optional(data, "datum_narozeni"))
    #field("Číslo OP:", to: 65%, content: optional(data, "op"))
  ...
  #field("Místo výkonu práce:", content: if optional(data, "tabor_od").len() > 0 { "Tábořiště Železné Horky v termínu od " + format-datetime(optional(data, "tabor_od")) + " do " + format-datetime(optional(data, "tabor_do"))})

  #field("Nástup dne:", content: if optional(data, "tabor_od").len() > 0 {
    format-datetime(optional(data, "tabor_od")) + " v Železných Horkách"
  })
